...
function addToCart(event){
    ...
    let itemId = btnGrandParent.children[1].value;
    ...
    <td><input type="hidden" class="Productid" value="${itemId}" /></td>
    <td><img class="uk-preserve-width uk-border-circle" src=${itemImage} width="100" alt=""></td>
    <td class="uk-table-link">
        <h3 class = "item-name">${itemName}</h3>
    </td>
    <td class="uk-text-truncate item-price"><h3>${itemPrice}</h3></td>
    <td><input   type = 'number' class = 'Quantity num' value = '1'></td>
...
function totalCost(event){
    ...
    price_field_content = price_field.innerText.replace('đ', '')
    total_field.children[0].innerText = (quantity.value * price_field_content).toFixed(2) + 'đ' 
    grandTotal()
...
function grandTotal(){
    ...
    for(let i = 0; i < all_total_fields.length; i++){
        all_prices = Number(all_total_fields[i].innerText.replace('đ', ''))
        total+=all_prices
    }
    grand_total.children[0].innerText = total.toFixed(2) + 'đ'
